...
def github_resp(**kwargs):
    ...
    try:
        sleep(randint(3, 10))
        r = get(url=f"{kwargs['url']}",
                auth=kwargs['login'],
                timeout=kwargs['timeout'],
                params=kwargs['params'],
                )
        r.raise_for_status()
    except exceptions.HTTPError as err_h:
        res.append(f"Http Error: {err_h}")
        return res
    except exceptions.ConnectionError as err_c:
        res.append(f"Error Connection: {err_c}")
        return res
    except exceptions.Timeout as err_t:
...
def valid_age(list_, number_days):
    ...
    bb = date(int(b11[0]), int(b11[1]), int(b11[2]))
    for i in list_:
        a1 = split(r':', i[1])
        a11 = a1[0].split('T')[0].split('-')
        aa = date(int(a11[0]), int(a11[1]), int(a11[2]))
